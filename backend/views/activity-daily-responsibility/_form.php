...
if ($Role == "Sekretariat") {
     $this->registerJs("
      ...
      $('.btn-save').on('click',function(){
        ...
        var id_realisasi = $('#activitydailybudgetsecretariat-budget_value_dp-disp').val();
        ...
        if(BigInt(budget_fix) < BigInt(realisasi_fix)){
            ...
            $('#activitydailybudgetsecretariat-budget_value_dp-disp').focus();
            return false;
        }
    ...
    $this->registerJs("
      ...
      $('.btn-save').on('click',function(){
        var id_budget_now = $('#budget_now').text();
        var budget_fix  = id_budget_now.replace('Rp ','').replace(/\./g,'');
        var id_realisasi = $('#activitydailybudgetsection-budget_value_dp-disp').val();
        var realisasi_fix = id_realisasi.replace('Rp ','').replace(/\./g,'');

        if(BigInt(budget_fix) < BigInt(realisasi_fix)){
            alert('Realisasi dana tidak boleh melebihi dana budget sekarang');
            $('#activitydailybudgetsection-budget_value_dp-disp').focus();
            return false;
        }
